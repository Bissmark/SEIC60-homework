...
// //3
// sum = system.Jupiter.satellites +system.Saturn.satellites
...
// //6
// diff = system.Jupiter.velocity * system.Jupiter.mass -
...
readingList = [
  ...
  //1c
  for (i in readingList){
    titleAuthor = `${readingList[i].title} by ${readingList[i].author}`
    if (readingList[i].alreadyRead === true){
      console.log(`You already read ${titleAuthor}`)
    } else {
      console.log(`You still need to read ${titleAuthor}`)
    }
  }